...
Ext.grid.filter.DateFilter = Ext.extend(Ext.grid.filter.Filter, {
    ...
    date.timestamp = menuItem.picker.getValue().getTime();
    var dates = this.dates;
	...
	setValue: function(value) {
		for(var key in this.dates) {
			if(value[key]) {
                this.dates[key].timestamp = value[key].getTime();
	...
	validateRecord: function(record) {
		var val = record.get(this.dataIndex).getTime();
        if (isNaN(val))
            return false;
		
		if(this.dates.on.checked && val != this.dates.on.timestamp) {
			return false;
    }
		if(this.dates.before.checked && val > this.dates.before.timestamp) {
			return false;
    }
		if(this.dates.after.checked && val < this.dates.after.timestamp) {
			return false;
    }
		return true;
	}
